...
template <typename T> template <typename U> vector <T>::vector (std::initializer_list <U> const & values) {
...
template <typename T> std::unique_ptr <iterator <T>> vector <T>::make_iterator () {
